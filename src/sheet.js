...
export function sheetRows(list) {
  ...
  for (const r of Array.isArray(list) ? list : []) {
    ...
    // 同一商品出現多列：保留優先序最高的狀態
    if (!prev || (STATUS_RANK[code] || 9) < (STATUS_RANK[prev.code] || 9)) out.set(key, { key, name, link, status, code });
  }
  return [...out.values()];
}

// 比對試算表與系統：新增、更新、下架、恢復
// existing: [{ id, sheet_key, name, link, sheet_status, status_code, delisted_at, deleted_at, source }]
export function planSync(existing, rows) {
  ...
  for (const r of rows) {
    ...
    if (!p) { plan.inserts.push(r); continue; }
